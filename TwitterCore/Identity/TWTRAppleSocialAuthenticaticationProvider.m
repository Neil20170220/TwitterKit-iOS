    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        for (ACAccount *account in self.accounts) {
            ...
            UIAlertAction *action = [UIAlertAction actionWithTitle:title
                ...
                [strongSelf getAuthTokenWithAccount:account completion:completion];
            }];
        ...
        UIAlertAction *action = [UIAlertAction actionWithTitle:NSLocalizedString(@"Cancel", nil)
                                                       ...
                                                       handler:^(UIAlertAction * _Nonnull action2) {
            NSError *error = [NSError errorWithDomain:TWTRLogInErrorDomain
                                                 code:TWTRLogInErrorCodeCancelled
                                             userInfo:@{ NSLocalizedDescriptionKey: @"User cancelled authentication." }];
            completion(nil, error);
        }];
        [controller addAction:action];
        
        [[TWTRUtils topViewController] presentViewController:controller animated:YES completion:nil];
    });
}
